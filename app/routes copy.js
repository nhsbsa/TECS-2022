...
router.post('/:version/:letter/partial-payment', function (req, res) {
  ...
  const originalTotal = parseFloat(
    req.session.data.originalTotalAmount || req.session.data.totalAmount
  );

  if (!rawAmount || rawAmount.trim() === '') {
    return res.render(`${version}/${letter}/partial-payment`, {
      data: { ...req.session.data, amountError: 'Enter an amount' }
    });
  }
  ...
  if (isNaN(amount) || amount <= 0) {
    return res.render(`${version}/${letter}/partial-payment`, {
      data: { ...req.session.data, amountError: 'Enter a valid amount' }
    });
  }

  const alreadyPaid = parseFloat(req.session.data.totalPaidSoFar || 0);
  const newTotalPaid = parseFloat((alreadyPaid + amount).toFixed(2));

  const remaining = parseFloat((originalTotal - newTotalPaid).toFixed(2));

  // 🔐 LOCK ORIGINAL AMOUNT ON FIRST USE
  if (!req.session.data.originalTotalAmount) {
    req.session.data.originalTotalAmount = originalTotal;
    req.session.data.originalTotalAmountFormatted = originalTotal.toFixed(2);
  }

  // CURRENT PAYMENT
  req.session.data.currentPaymentAmount = amount;
  req.session.data.currentPaymentAmountFormatted = amount.toFixed(2);

  // TOTALS
  req.session.data.totalPaidSoFar = newTotalPaid;
  req.session.data.totalPaidSoFarFormatted = newTotalPaid.toFixed(2);

  req.session.data.remainingAmount = remaining;
  req.session.data.remainingAmountFormatted = remaining.toFixed(2);

  // IMPORTANT: DO NOT MUTATE ORIGINAL TOTAL FLOW
  req.session.data.totalAmount = remaining;
  req.session.data.totalAmountFormatted = remaining.toFixed(2);

  req.session.data.paymentAmount = 'partial-amount';
